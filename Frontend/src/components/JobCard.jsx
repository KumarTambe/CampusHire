            ...
            <Card hover className="animate-rise stagger flex h-full flex-col p-5" style={{ '--i': index }}>
                <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-3">
                        <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-indigo-500/18 to-violet-500/8 text-indigo-300 ring-1 ring-inset ring-white/10">
                            <IconBuilding className="size-[18px]" />
                        </span>
                        <div className="min-w-0">
                            <h3 className="line-clamp-2 text-[15.5px] font-semibold leading-snug text-white transition-colors group-hover:text-indigo-200">
                                {job.title}
                            </h3>
                ...
                <p className="mt-4 line-clamp-2 text-[13.5px] leading-relaxed text-slate-400">
                    {job.description}
                </p>
                ...
                {job.requiredSkills?.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-1.5">
                        {job.requiredSkills.slice(0, 4).map(skill => (
                            <span key={skill}
                                className="rounded-md bg-white/[0.05] px-2 py-1 text-[11px] text-slate-400 ring-1 ring-inset ring-white/8">
                                {skill}
                            </span>
                        ))}
                ...
                <div className="mt-auto flex flex-wrap items-center gap-x-4 gap-y-2 pt-5 text-[12px] text-slate-500">
                    ...
                    <span className="ml-auto inline-flex items-center gap-1 text-indigo-400 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100 -translate-x-1">
